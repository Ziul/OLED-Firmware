...
//*****************************************************************************
#pragma inline
unsigned char NumLeadingZeros(unsigned long x) 
{  
int i=0;
   for (i ; i<32 ;i++)
      if (x&(1<<(31-i)))
...
GrStringDraw(const tContext *pContext, const char *pcString, int lLength,
             unsigned lX, unsigned lY, int bOpaque)
{
    unsigned lIdx, lX0, lY0, lCount, lOff, lOn, lBit;
    const unsigned char *pucData;
    tContext sCon;
...
#define SC_IS_NULL              0xffff
#define SC_GET_LEN(v)           ((v) >> (15 - 5))
#define SC_GET_INDEX(v)         (((v) >> 8) & SC_MAX_INDEX)
#define SC_GET_OFF(v)           ((v) & SC_IS_NULL)

#define SC_FLAG_COMPRESSED      0x8000
#define SC_OFFSET_M             0x7fff
...
//
//*****************************************************************************
static const unsigned *g_pulStringTable;
...
GrStringTableSet(const void *pvTable)
{
    ...
    //
    g_pulStringTable = (unsigned *)(g_pusLanguageTable +
                                         g_usNumLanguages);
...
//*****************************************************************************
unsigned 
GrStringGet(int iIndex, char *pcData, unsigned ulSize)
{
    unsigned ulLen, ulOffset, ulSubCode[16];
    ...
    if(pucBufferOut)
    {
        ulLen = ((unsigned)pucBufferOut - (unsigned)pcData);
